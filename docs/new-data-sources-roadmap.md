...
| GeoNet (New Zealand) | New Zealand | Earthquakes, volcanic alert level, landslides, tsunami | `api.geonet.org.nz/quake?MMI=0` JSON (CORS `*`); no key | Free, CC-BY (cite DOIs) | Very active tectonics/volcanoes; browser-friendly | ✅ Earthquake feed active; volcano, landslide, and tsunami products remain candidates |
